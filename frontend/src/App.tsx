...
function HomeView({ tick, toast, following }: Ctx & { goTab: (t: any) => void; following: string[] }) {
  ...
  // your own posts always belong in "Following"; the rest comes from the real follow graph
  const followSet = new Set<string>([...following, "zeeshan"]);
  ...
  const shown = posts.filter((p) =>
    (feedTab === "foryou" || followSet.has(p.author_handle)) &&
    ...
    <div className="feedsolo">
      <div className="feedmain">
        ...
        <div className="feedtabs">
          <button className={feedTab === "foryou" ? "on" : ""} onClick={() => setFeedTab("foryou")}>For you</button>
          <button className={feedTab === "following" ? "on" : ""} onClick={() => setFeedTab("following")}>Following</button>
          <div className="platpills">
            {platforms.map((pf) => (
              <button key={pf} className={"platpill" + (plat === pf ? " on" : "")} onClick={() => setPlat(pf)}>
                {pf === "all" ? "All platforms" : <><span className={`acc ${pf}`} style={{ width: 16, height: 16, borderRadius: 5, fontSize: 8 }}>{icon(pf)}</span> {pf}</>}
              </button>
            ))}
          </div>
        </div>
          ...
          <div className="fpost" key={p.id}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div className="fmeta">
                ...
                <span className={`fplat ${p.platform}`} title={`posted on ${p.platform}`}>{p.platform === "wolfie" ? "🐺" : icon(p.platform)}</span>
              </div>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div className="profplats">
            {p.platforms.map((pf: string) => (
              <span key={pf} className={`acc ${pf}`} title={pf} style={{ width: 26, height: 26, borderRadius: 8, fontSize: 12 }}>{icon(pf)}</span>
            ))}
          </div>
        </div>
        ...
        {shown.map((x) => (
          <div className="card p profpost" key={x.id}>
            <div className="fmeta" style={{ marginBottom: 8 }}>
              ...
              {x.platform && <span className={`fplat ${x.platform}`}>{icon(x.platform)}</span>}
            </div>
          ...
          <div className="synced">
            {a.synced.map((s: any) => (
              <div className="syncrow" key={s.platform}>
                <span className={`acc ${s.platform}`} style={{ width: 24, height: 24, borderRadius: 7, fontSize: 11 }}>{icon(s.platform)}</span>
